...
Definition PI := forall P:Prop, forall p q:P, p = q.

Definition bproj := fun b => match b with true => ptrue | false => pfalse end.

(* Covnerse note possible because can't destruct prop for a type (and we don't hav ethe singleton criteria) *)

Lemma sur_not_pi : (exists (f: pbool -> bool), (forall x:bool, exists p:pbool, f p = x )) -> ~PI.
Proof.
    intros [f hf].
    intros hPI. unfold PI in hPI.
    destruct (hf true) as [pt Hft].
    destruct (hf false) as [pf Hff].
    change (if true then False else True).
    rewrite <- Hft.
    rewrite (hPI pbool pt pf).
    rewrite Hff.
    constructor.
Qed.
...
Lemma ok: forall p:pbool, exists b:bool, bproj b = p.
Proof.
    intros p.
    destruct p.
...
Lemma ex_to_prop {A} {P:A -> Prop} : { x : A | P x } ->exists (x : A), P x.
Proof.
    intros h. destruct h as [x hP]. exists x. exact hP.
Qed.
...
Lemma or_to_prop {A B:Prop} : A + B -> A \/ B.
Proof.
    exact (fun opt => match opt with | inl x => or_introl x | inr x => or_intror x end).
Qed.  
...
Lemma case_inv {f: bool -> bool}: (exists x, f x = true) -> { x : bool | f x = true }.
Proof.
    intro h.
    
    destruct (f true) as ht.
    - 
